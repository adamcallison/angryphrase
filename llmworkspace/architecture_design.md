...
| Module | Owns |
|---|---|
| `app/state/intents.ts` | `AppIntent` union: `navigate`, `cancel-modal`, `dismiss-toast`, `report-download-failure`, `report-builder-save-failure`, `report-player-save-failure` (no `confirm-modal` — confirm dispatches the specific `confirm-*` Builder/Player intent directly) |
...
| Module | Owns |
|---|---|
| `appStore.svelte.ts` | `createAppStore(initial, deps, ports, options?): AppStore` factory + `AppStore` / `AppDeps` / `AppPorts` types (§2.4) — the sole `$state` cell lives in the factory closure; `reduceApp` dispatch work-queue; external-event performance via the injected `AppPorts`; constructs the `PersistenceScheduler` internally from `ports.storage` (P3/W1 — closes the scheduler↔store dependency cycle and guarantees the scheduler's storage is the store's storage) and surfaces storage write failures per slice via the scheduler's `onWriteResult` channel (warn once per episode + `report-*-save-failure` toast, write-driven re-toast cadence while failing, re-armed by the next successful write); route/toast/modal getters. The only rune-using file in the layer (hence the only `.svelte.ts`) |
...
| `persistenceCodec.ts` / `persistenceScheduler.ts` | pure codec (unchanged); `createPersistenceScheduler(storage, debounceMs, onWriteResult)` — the scheduler is constructed **inside** `createAppStore` from `ports.storage` (P3/W1), never passed in from outside. Mechanical only: debounce, serialize, coalesce, cancel-on-clear, flush. After every *attempted* write (save or clear, per slice, including `flush()`) it reports `onWriteResult(slice: 'builder' | 'player', err: Error | null)`; skipped saves (import-phase player) report nothing. It holds no failure policy — latch / warn / toast / re-arm live in the appStore |
...
**Layer 5 — `main.ts`.**
Boots the app: instantiates the production ports directly from `ports/` (no register — the `ports.ts` singleton is deleted, §2.4), loads initial `AppState` (Builder state from `localStorage` or a fresh blank puzzle per FR-19; player starts at the import screen), constructs `AppDeps` (`{ rng, now }`) and `AppPorts` (`{ storage, download, filePick }`), calls `createAppStore(initial, deps, ports, options?)` exactly once — the `PersistenceScheduler` is constructed *inside* the store from `ports.storage` (P3/W1); `main.ts` never touches it — and mounts `<App appStore={store} />` to `#app`. Wires the persistence knobs as optional `createAppStore` config — `options.persistenceDebounceMs` (default 400 ms) and `options.saveFailureRetoastMs` (default 60 s, P3) — per the "inject as config value" principle from F2; `main.ts` runs on the defaults. `main.ts` is the only composer — nothing else constructs an `AppStore` in production.
...
- **the `AppStore` instance performs external side effects** (via its injected `AppPorts` + internally-constructed `PersistenceScheduler`): after `dispatch(intent)` returns `{ state, events }`, the `AppStore` instance first folds `result.events` through `applyEventsToApp` (consuming `toast`/`modal-request` into `state.toasts`/`state.modal`; passing `download`/`clear-builder-storage`/`clear-player-storage`/`load-player-progress` through as `leftoverEvents`), sets the rune to the folded state (which causes reactive VM updates), then iterates the `leftoverEvents`. For each leftover event: `download` → calls `downloadPort.download(filename, content)`; if it returns an `Error`, the bindings layer dispatches `report-download-failure` (G7 — `reduceApp` turns it into an error toast so a failed irreversible user action no longer disappears silently; mirrors the `load-player-progress → apply-loaded-progress` follow-up pattern); `clear-builder-storage` → calls `storagePort.clearBuilder()`; `clear-player-storage { key }` → calls `storagePort.clearPlayerProgress(key)`; `load-player-progress { key }` → calls `storagePort.loadPlayerProgress(key)` and dispatches `apply-loaded-progress` (see §4.4). The fold step is what lets an AppIntent like `report-download-failure` return a `toast` event and have it reach `state.toasts` — without the fold, `performExternalEvent`'s `case 'toast': return null` would drop it. (For Builder/Player intents the fold is a no-op pass-through: `reduceApp` already folded their events internally and returns only the leftover external events, so re-folding leftover events returns them unchanged.) Toast auto-dismiss timeouts likewise dispatch `dismiss-toast` intents, never direct mutations. **Storage write failures (P3/W1):** the scheduler reports every attempted write via `onWriteResult(slice, err)`; the store latches per slice — on the first failure of an episode it warns once and dispatches `report-builder-save-failure` / `report-player-save-failure` (an error toast naming the affected slice); the latch re-arms only after a subsequent successful write in that slice, so persistently failing storage surfaces one toast per failure episode onset rather than one per 400 ms debounce tick, and re-toasts write-driven at a configurable cadence (`options.saveFailureRetoastMs`, default 60 s) while the failure persists. Failed clears arm the same latch; for the Builder the next successful save overwrites the stale blob (self-healing); a failed `clearPlayerProgress(key)` leaves a stale keyed blob that resurrects only if that exact puzzle is re-imported (accepted edge).
...
export function createAppStore(
  ...
  deps: AppDeps,
  ports: AppPorts,
  options?: { persistenceDebounceMs?: number; saveFailureRetoastMs?: number },  // P3/W1: the scheduler is constructed internally from ports.storage; re-toast cadence default 60 s
): AppStore;
```

`dispatch` is the work-queue loop over `reduceApp` → `applyEventsToApp` fold → external-event performance (§2.2), reading only closure-scoped `state` / `deps` / `ports` / `scheduler`. `handleLoadPlayerProgress` reads `ports.storage`. The `scheduler` is constructed inside the factory over `ports.storage` (P3/W1) — its `onWriteResult` reports feed the store's per-slice save-failure latches (§2.2). No `bootApp`, no `_resetAppStateForTests`, no `getPorts()` register — the `ports.ts` file is deleted; `main.ts` instantiates the production ports directly from `ports/` and passes them as the `AppPorts` argument.
...
export interface StoragePort {
  loadBuilder(): string | null;                       // raw JSON string of BuilderState blob; null if missing or unreadable (warn on failure)
  saveBuilder(blob: string): Error | null;            // P3 — null = success; Error = write failed (silent; appStore warns/toasts)
  clearBuilder(): Error | null;                       // P3 — same contract
  loadPlayerProgress(key: PuzzleKey): string | null;  // null if missing or unreadable (warn on failure)
  savePlayerProgress(key: PuzzleKey, blob: string): Error | null;  // P3 — same contract
  clearPlayerProgress(key: PuzzleKey): Error | null;  // P3 — same contract
}
...
export type AppIntent =
  ...
  | { kind: 'report-builder-save-failure' };                  // P3 — appStore raised on the first failed Builder-slice storage write of a failure episode (latch re-armed by the next successful write)
  | { kind: 'report-player-save-failure' };                   // P3 — same, Player slice
...
7. If `intent` is `report-builder-save-failure` (P3): return `{ state, events: [{ kind: 'toast', toastKind: 'error', message: 'Saving failed — your Builder changes are not being kept. Download an export to keep a copy.' }] }`. State unchanged.
8. If `intent` is `report-player-save-failure` (P3): return `{ state, events: [{ kind: 'toast', toastKind: 'error', message: 'Saving failed — your solving progress is not being kept.' }] }`. State unchanged.
...
1. **State observation (autosave).** `App.svelte` runs two `$effect`s over its `appStore` prop (§2.4 item 7) — one over `state.builder` (the full BuilderState) and one over `state.player` (when `phase === 'solving'`) — split per-slice so a change to one slice does not re-arm the other's debounce timer. On any change, debounce (configurable; default 400 ms per F2) and call `storagePort.saveBuilder(blob)` or `storagePort.savePlayerProgress(key, blob)`. Writes return `Error | null` (P3); the scheduler reports every attempted write via `onWriteResult(slice, err)` to the appStore, which latches per slice — warn once + `report-builder-save-failure` / `report-player-save-failure` error toast on the first failure of an episode, re-armed by the next successful write in that slice (one toast per debounce tick is prevented by the latch). While a latch stays armed, the toast re-fires **write-driven at a configurable cadence** (`options.saveFailureRetoastMs`, default 60 s, P3/b): the next failed write that lands ≥ N after that slice's last toast re-toasts. Reminders ride on attempted writes — an idle session attempts no writes and loses nothing new, so no wall-clock timer is needed. The persisted Builder blob is richer than the incomplete-puzzle JSON — it includes `mode` and `subMode` for restore — so it is a wrapper around the puzzle JSON, not the puzzle JSON directly:
   ```ts
...
2. **Event-driven storage clears.** When the bindings layer receives a `{ kind: 'clear-builder-storage' }` event (from `confirm-reset-builder`), it calls the scheduler's `clearBuilder()` *synchronously* (cancelling any pending Builder save) which calls `storagePort.clearBuilder()` before any debounced autosave fires. Same for `{ kind: 'clear-player-storage'; key }`. Clear results flow through the same `onWriteResult` channel (P3): a failed clear arms the slice's failure latch (toast). For the Builder the next successful autosave overwrites the stale blob (self-healing); a failed `clearPlayerProgress(key)` leaves a stale keyed blob that resurrects only if that exact puzzle is re-imported (accepted edge).
...
| `AnagramModal.svelte` | `AnagramModalVM` | `anagram-input`, `anagram-scramble`, `close-anagram-helper` | Modal per FR-81..FR-89. Closes on backdrop click / Escape / selection-change (FR-88 / FR-89). No grid write-back (FR-87). The textbox owns an `onkeydown` Escape handler that dispatches `close-anagram-helper` (preserves FR-89 now that the typing surface is disabled while the modal is open), and a local `$effect` focuses the textbox when `vm.open` becomes true so the player can type immediately on open. |
...
**Ports allow-list convention (I1).** The `src/ports/**` block uses a negative-lookahead allow-list (unavoidable — `no-restricted-imports` has no allow-list primitive, only restrict patterns) enumerating the 3 allowed paths with `.ts` OPTIONAL. This matches every other layer block's extensionless prefix-regex idiom; imports may carry `.ts` but are not required to. `tsconfig.json` `allowImportingTsExtensions` was retired as part of I1 (its sole rationale was the former `.ts`-required ports regex).
