...
| `BuilderCluePanel.svelte` | `CluePanelVM` | `click-clue-panel-word`, `begin-join`, `unjoin`, `edit-clue` | Composes two `<ClueSection title="Across"/"Down">` (B3). Owns scroll-into-view `$effect` + `isInJoinMode` derived; per-row dispatch + `drafts` Map live in `ClueSection`. Sections Across/Down sorted by number (FR-32). Chain heads have an editable text input (FR-31); non-heads show "See N Direction" reference, no input. Per-clue "Link next" / "Unlink" controls (FR-38) when relevant. Scrolls the highlighted clue into view (FR-32). |
| `ClueSection.svelte` | `title: string; entries: ClueEntryVM[]; isInJoinMode: boolean` | `click-clue-panel-word`, `begin-join`, `unjoin`, `edit-clue` | Single Across-or-Down section (B3 extraction). Owns the per-row `drafts` Map + all per-row dispatch helpers; renders the `<section><h2><ul>{#each entries}</ul></section>` body. DOM id `${direction}-${number}` preserved for parent scroll effect. |
...
│  │  │  ├─ BuilderGrid.svelte  BuilderCluePanel.svelte  ClueSection.svelte
