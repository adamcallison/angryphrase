...
- **FR-28.** Markers indicate a separator **after** the cell in the named direction: a space-separator or a hyphen-separator between the current cell and the next cell of a word — or, when the cell is the **last cell of a word that has a `nextWord` link** (a non-tail chain member), between that word and the linked word (the **chain boundary**; the gap between the source's last letter and the successor's first letter in the clue's reading order). Markers are visual (a vertical/horizontal bar for spaces; a hyphen glyph for hyphens) and are used in **length-pattern** display (§3.24). An empty direction pair on a chain boundary means **no separator** (the length runs merge across the boundary; amended 2026-09-09 — see `llmworkspace/flexible_multiword_separator.md`).
...
- **FR-36.** **Successful join effect:** the source's `nextWord` is set to the target. If the target had a non-empty clue, that clue is **displaced** into the **Displaced Clues** panel (§3.11). A target with an empty clue produces no displaced clue. Additionally, the **default space separator is materialized** on the source's last cell in the source's direction (per FR-28): if the direction pair there is empty, the space flag is set; a pre-existing space or hyphen flag is respected, not overwritten (amended 2026-09-09).
- **FR-37.** **Unjoin:** The builder may remove a word's outgoing `nextWord` link. Effects: the source's `nextWord` is cleared; the source's **chain-boundary marker pair is cleared** (per FR-28 — the marker's meaning exists only while the link exists); the formerly-linked (downstream) word — which had been displaying a chain reference — becomes an independent word and gets its clue **reset to empty**. Only the direct link is removed: if A→B→C and A is unjoined, B retains its link to C (amended 2026-09-09).
...
- **FR-91.** **Length pattern for a word** (used in the clue list and the active-clue banner; wrapped in parentheses in display):
  - If the word **has a `nextWord` link** (i.e., it is a chain head with a link, or a mid-chain non-head with a link), the pattern is built by walking the chain from this word to the tail as a **virtual single word**: one flat cell sequence with a run counter shared across the whole sequence. Within each member, that member's own `spaceRight`/`spaceBottom`/`hyphenRight`/`hyphenBottom` markers split its own runs per the no-`nextWord` rule below (a member's `nextWord` is **not** consulted, so there is no recursion into the chain branch). **Between** member `i` (non-last) and member `i+1`, the **chain-boundary marker** (FR-28) on member `i`'s last cell decides: a space marker emits run + `", "`; a hyphen marker emits run + `"-"`; an empty pair merges the run across the boundary (no emission). The final run is emitted at the tail's last cell. Example: a 3-member chain of plain words `4, 4, 5` with space boundaries renders `4, 4, 5`; with hyphen boundaries `4-4-5`; with empty boundaries `13`; a chain of `5, 9, 7` with space boundaries whose tail carries a `spaceRight` after its 2nd cell (splitting it 2+5) renders `5, 9, 2, 5`; with a hyphen boundary after the first member and an empty boundary after the second, `5-16`. The pattern reflects the **suffix of the chain from this word onward**, not necessarily the whole chain from its head. (Amended 2026-09-09 — replaces the former fixed `", "` member join; see `llmworkspace/flexible_multiword_separator.md`.)
...
- **FR-94.** The puzzle JSON format versions are **1 and 2**. Both formats include a `version` field equal to 1 or 2 and a `type` discriminator (`"incomplete"` or `"complete"`). The parser shall reject files with an unrecognized version or type. The two versions share an identical schema (no fields are added, removed, or renamed); only the **interpretation of an empty chain-boundary marker pair** differs — in a version 1 file an empty boundary pair on a linked word's last cell is normalized to a space marker (the pre-feature implicit default; see FR-99), while in a version 2 file an empty boundary pair means **no separator** and round-trips unchanged. The application **writes version 2**. (Amended 2026-09-09.)
...
- **FR-99.** The parser/validator shall, where reasonable, **normalize** the input by filling in default values (e.g., missing marker booleans default to `false`; the player letter is always initialized to `null` on import; in **version 1** files only, an empty chain-boundary marker pair on a linked word's last cell defaults to a space marker per FR-94). The parser shall produce a clear, single error message listing the validation failures on rejection.
