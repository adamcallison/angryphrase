...
- **FR-91.** **Length pattern for a word** (used in the clue list and the active-clue banner; wrapped in parentheses in display):
  - If the word **has a `nextWord` link** (i.e., it is a chain head with a link, or a mid-chain non-head with a link), the pattern is built by walking the chain from this word to the tail and, for each member, computing that member's **separator-aware sub-pattern** per the no-`nextWord` rule below (a member's own `spaceRight`/`spaceBottom`/`hyphenRight`/`hyphenBottom` markers split its own runs — a member's `nextWord` is **not** consulted, so there is no recursion into the chain branch); the member sub-patterns are then joined by `", "` (comma + space). Example: a 3-member chain of plain words renders `4, 4, 5`; a chain of `5, 9, 7` whose tail carries a `spaceRight` after its 2nd cell (splitting it 2+5) renders `5, 9, 2, 5`. The pattern reflects the **suffix of the chain from this word onward**, not necessarily the whole chain from its head.
