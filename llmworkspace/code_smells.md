...
### G7. `DownloadPort.download` is `void`-returning; both port impl and `appStore` swallow errors with no user feedback 🟠 ✅ Resolved
`src/domain/ports/ports.ts:12-13` `download(filename, content): void` — no `Result` channel. `src/ports/downloadPort.ts:6-18` wraps the DOM Blob/anchor dance in `try/catch` → `console.warn` only. `src/ui/bindings/appStore.svelte.ts:75-81` adds a second `try/catch` that also just `console.warn`s. Effect: a user-initiated download that fails (rare, but possible: blocked focus, sandbox, store API throw) leaves the user with zero UI signal — no toast, no modal, no inline state change. Failed irreversible user action disappears silently.

#### G7 — Fix (two tasks, serialized)
- **Task A — port layer**: widened `DownloadPort.download` return type `void` → `Error | null` (null = success; `Error` = failure). `src/ports/downloadPort.ts` impl: try/catch now `return err instanceof Error ? err : new Error(String(err))` on caught error (defensive coercion for non-Error throws), `return null` on success; removed the impl's `console.warn` (logging responsibility moved to the bindings layer which has dispatch context). `test/fakes/StubDownloadPort.ts`: `download` returns `null` by default; added `public nextDownloadError: Error | null = null` field for failure injection — when set, `download` records the call, consumes the injected error once, and returns it; `reset()` clears both `downloads` and `nextDownloadError`. `test/fakes/StubDownloadPort.test.ts` + `test/ports/downloadPort.test.ts`: updated success-path tests to assert `null` return; rewrote the "URL.createObjectURL missing" test from "does not throw" → "returns an `Error`"; added StubDownloadPort failure-injection test.
- **Task B — state + bindings wiring**: new `AppIntent` variant `{ kind: 'report-download-failure' }` in `src/app/state/intents.ts`. `src/app/state/reducer.ts` `reduceApp` new case: `return { state, events: [{ kind: 'toast', toastKind: 'error', message: 'Download failed. Please try again.' }] }` — state unchanged, toast event emitted. `src/ui/bindings/appStore.svelte.ts` `performExternalEvent` `case 'download'`: replaced the try/catch swallow with `const err = getPorts().download.download(...); if (err !== null) { console.warn(...); return { kind: 'report-download-failure' }; } return null;` — port no longer throws (Task A), so no try/catch; `console.warn` retained at the bindings layer (dispatch context). Return type widened `PlayerIntent | null` → `AppIntent | BuilderIntent | PlayerIntent | null` so the new AppIntent follow-up is assignable. **Dispatch-loop fold step (required, discovered during Task B)**: the existing `dispatch` work-queue loop iterated `result.events` directly via `performExternalEvent`, whose `case 'toast': return null` would drop the `toast` event from `report-download-failure` before it reached `state.toasts`. Added `const folded = applyEventsToApp(result.state, result.events, d); s = folded.state;` before iterating `folded.leftoverEvents` — this folds `toast`/`modal-request` into state and passes `download`/`clear-*`/`load-player-progress` through as leftover. For Builder/Player intents the fold is a no-op pass-through (`reduceApp` already folded their events internally and returns only leftover external events, so re-folding returns them unchanged); for AppIntents that return `toast` events (`report-download-failure`), the fold is what makes the toast reach `state.toasts`. Mirrors the F6 work-queue follow-up pattern. `applyEventsToApp` imported from `app/state/effects`.
- **Design choice — `Error | null` not branded `DownloadError`**: the smells doc's suggested `Result<null, DownloadError>` / `DownloadError` discriminated union was rejected. `Error` is a built-in class (AD §0 Principle 4 bans plain `string`/`number`, not `Error`); the bindings layer doesn't inspect structurally (just `!== null`); a branded type adds construction boilerplate for a failure path that's extremely rare. `Error | null` is consistent with the project's nullable return idiom (`parsePlayerProgress` returns `T | null`).
- AD amendments: §1.3 module table (`intents.ts` row adds `report-download-failure`), §3.5a (`DownloadPort.download` signature widened), §4 line 217 (bindings-layer event iteration now documents the fold step + download-failure follow-up), §4.1 step 6 (new `reduceApp` responsibility for `report-download-failure`).
- New tests: `test/app/state/reducer.test.ts` `'reduceApp: report-download-failure emits an error toast event, state unchanged'` (asserts state ref-equal + `events[0]` deep-equals the toast event); `test/ui/bindings/appStore.test.ts` `'appStore: download failure surfaces an error toast (StubDownloadPort injected to return Error)'` (injects `nextDownloadError`, dispatches `export-complete`, asserts 1 download recorded + 1 error toast with exact message); `test/app/state/intentKinds.test.ts` AppIntent-kinds test updated to include `report-download-failure` in the not-in-any-set loop; `test/fakes/StubDownloadPort.test.ts` failure-injection test. Existing 17 `appStore.test.ts` + download success tests unchanged and green.
- Verification (green): `npm run typecheck` (svelte-check 0 errors / 0 warnings), `npm run lint` (eslint + `madge --circular` clean), `npm run test` (77 files / 1035 tests), `npm run ci` green. `grep -rn 'report-download-failure' src/ test/` → 6 files (`intents.ts`, `reducer.ts`, `appStore.svelte.ts`, `intentKinds.test.ts`, `reducer.test.ts`, `appStore.test.ts`). `grep -n 'void' src/domain/ports/ports.ts src/ports/downloadPort.ts test/fakes/StubDownloadPort.ts` → zero `void` return annotations on `download` (StoragePort methods keep `void`).
- Out of scope (separate tasks): H1-H4 (branded types — H1 `brand<>` bypass, H2 `withinBounds` idiom, H3 pervasive `!`, H4 plain `number`), I1 (eslint regex), J1-J3 (domain logic quirks), K2/K3 (docs).
...
### H1. `brand<Tag, T>` is structurally unsound at the brand boundary 🟠 ✅ Resolved
...
#### H1 — Fix
- **Ethos**: AD §0 Principle 4 + §1 B1 promise branded types make illegal values unconstructable at the boundary. The `brand()` escaper is an unsound cast that trusts the caller. The fix closes all non-owner bypasses, then makes the closure durable via lint + self-test — same enforcement pattern as A1 (ESLint `no-restricted-imports` + `test/boundary/imports.test.ts`).
- **Task 1 — `persistenceCodec.ts` bypass closed**: `parsePlayerProgress` line 128 `brand<'PuzzleKey', string>((parsed as { key: string }).key)` → `PuzzleKey.try(...)`; null → `console.warn` + `return null` (NFR-9 corrupt-drop, matches `parseBuilderSnapshot` idiom). Line 4 type-only import → value import; line 8 `brand` import deleted. 1 new test: `'parsePlayerProgress: invalid non-UUID key returns null (H1 corrupt-drop)'`.
- **Task 2 — `v1.ts` bypass closed**: `validateDisplacedClues` return type changed from `{ id: string; ... }[]` to `{ id: DisplacedClueId; ... }[]`; `DisplacedClueId.try(id)` result captured as `validatedId` and threaded forward (pushed into `result`); the mapper at line 645 `brand<'DisplacedClueId', string>(d.id)` → `d.id` (already branded). Line 22 `brand` import deleted. No new tests (35 existing v1 tests + C3 UUID-rejection cover the path).
- **Task 3 — `Direction.advance` bypass closed**: lines 20-21 `brand<'Col'/'Row', number>(coord.col/row + n)` → `Col.of(coord.col + n)` / `Row.of(coord.row + n)`. Line 1 `brand` import deleted; lines 2-3 `import type { Row/Col }` → value imports. No new tests (7 existing Direction tests cover advance + negative-n; coords stay ≥0 so `Row.of`/`Col.of` don't throw).
- **Task 5 — Lint enforcement + self-test**: `eslint.config.js` extended. Brand-ban `no-restricted-imports` pattern (`(?:^src/domain/brand(?:\.ts)?$|(?:\.\./)+domain/brand(?:\.ts)?$|(?:\.\./)+brand(?:\.ts)?$)`) appended to every per-glob block (builder, player, app, ui non-bindings, ports). `src/domain/**` block split: non-owners get the brand ban (with `ignores` listing the 11 owners); owners get a separate block with the original 6 patterns and no brand ban. New `src/ui/bindings/**` block (brand ban only — bindings is ignored by the ui non-bindings block). New catch-all block for `src/**` not covered by any existing block (e.g. `src/main.ts`). `test/**` is out of scope — tests legitimately need `brand()` for edge-case fixtures. `test/boundary/imports.test.ts` gained 4 brand-specific fixtures: owner allowed, non-owner domain banned, bindings banned, `test/**` allowed (scope control). Total fixtures 36 → 40.
- **No production code added for test convenience**: the original plan proposed `ToastId.try` so test fixtures could drop `brand()`; rejected because it would shape a production API purely for test convenience. Tests retain the `brand()` escaper; the lint rule scopes to `src/**` only.
- AD §2.3 + §9.2 amended: brand-import boundary + enforcement mechanism documented.
...
- `DownloadPort.download` silent-failure path: no `Result` channel, double-swallowed errors, zero UI feedback on failed irreversible user action — see G7. ✅ Resolved (G7 — `DownloadPort.download` widened to `Error | null`; new `report-download-failure` AppIntent → error toast; dispatch-loop fold step added).
