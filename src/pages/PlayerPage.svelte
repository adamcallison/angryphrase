<script lang="ts">
  ...
  import AnagramHelper from "../components/AnagramHelper.svelte";
  ...
  function handleOpenAnagram(): void {
    anagramOpen = true;
  }

  function handleCloseAnagram(): void {
    anagramOpen = false;
  }
</script>
...
{:else}
  <HiddenInput bind:inputRef={keyboardInput} onKeyAction={handleKeyAction} />
  <div class="min-h-screen bg-slate-50">
    ...
    <div class="max-w-7xl mx-auto px-4 py-4">
      <div class="flex flex-col lg:flex-row gap-6">
        <!-- Left: Grid area -->
        <div class="flex flex-col" style="width: calc({playerData.grid.length} * var(--cell-size))">
          <ActiveClueDisplay
  ...
  {#if anagramOpen && selectedWord}
    <AnagramHelper
      word={selectedWord}
      grid={playerData.grid}
      onClose={handleCloseAnagram}
    />
  {/if}
{/if}