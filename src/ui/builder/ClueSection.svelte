<script lang="ts">
  import type { ClueEntryVM } from '../bindings/viewmodels/cluePanelVM';
  import { dispatchBuilder } from '../bindings/builderStore.svelte';

  let { title, entries, isInJoinMode }: {
    title: string;
    entries: ClueEntryVM[];
    isInJoinMode: boolean;
  } = $props();
...
<section>
  ...
  <ul class="flex flex-col gap-1">
    {#each entries as entry (canonicalId(entry.wordKey))}
